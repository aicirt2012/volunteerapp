...
router.get('/', function (req, res) {
    Event.find2('find(event).select({id:id, title: title, nrhelpers:nrhelpers, nrhelpersregistered: helpers.count(), startdate:startdate, enddate:enddate, place:place, isseries:isseries, highpriority:highpriority, organization: {name: organization.name, street: organization.street, zip:organization.zip}})', function (err, events) {
        res.json(events);
    });
});
